...
namespace Proje
{
    public partial class frmPersonelDurum : Form
    {
        public frmPersonelDurum()
        {
            InitializeComponent();
        }
        ...
        {
            baglanti.Open();
            string kayit = "SELECT * from yonetici";
        ...
        private void frmPersonelDurum_Load(object sender, EventArgs e)
        {
            kayitGetir();
        }
        ...
        {
            baglanti.Open();
            string secmeSorgusu = "SELECT * from yonetici where ID=@ID";
            ...
            if (dr.Read()) //Datareader herhangi bir okuma yapabiliyorsa aşağıdaki kodlar çalışır.
            {
                ...
                {
                    string silmeSorgusu = "DELETE from yonetici where ID=@ID";
