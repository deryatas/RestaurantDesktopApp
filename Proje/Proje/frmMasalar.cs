...
namespace Proje
{
    public partial class frmMasalar : Form
    {
        ...
        private void frmMasalar_Load(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("Select DURUM,MASANO from masalar", con);
            ...
            dr = cmd.ExecuteReader(); while (dr.Read())
            {
                foreach (Control item in this.Controls)
                {
                    if (item is Button)
                    {
                        if (item.Name == "btnMasa" + dr["MASANO"].ToString() && dr["DURUM"].ToString() == "REZERVASYONLU")
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.YENİLE__1_);
                        }
                        else if (item.Name == "btnMasa" + dr["MASANO"].ToString() && dr["DURUM"].ToString() == "DOLU")
                        {
                        ...
                        else if(item.Name == "btnMasa" + dr["MASANO"].ToString() && dr["DURUM"].ToString() == "BOŞ")
                        {
